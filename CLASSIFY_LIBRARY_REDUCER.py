#!/usr/bin/python
import sys
def CLASSIFY_LIBRARY_REDUCER():
    '''this method act as reducer and will get key value from mapper. It classifies each tweet and then assigns catagory to it\n
    which is later used to classify each library.'''
    cat1=[]
    cat2=[]
    cat3=[]
    cat4=[]
    ...
    l2 = ["u'Oregon State Library'",
          "u'Hamilton PL'",
          "u'Texas State Library'",
          "u'AustinPublicLibrary'",
          "u'Yale Library'",
          "u'NY Public Library'",
          "u'Beinecke Library'",
          "u\"Washington's Library\"",
          "u'OkStateLibrary'",
          "u'Eugene Library'"]
    ...
    for line in sys.stdin:
        ...
        try:
            ...
            # print '%s:::::%s' % (words[0].split('|')[2]+'|'+max(mapOfcat, key=mapOfcat.get), words[1].strip())
            if str(words[0].split('|')[2] + '|' + max(mapOfcat, key=mapOfcat.get)).strip() in outPut1:
                outPut1[str(words[0].split('|')[2] + '|' + max(mapOfcat, key=mapOfcat.get)).strip()] = outPut1[str(
                    words[0].split('|')[2] + '|' + max(mapOfcat, key=mapOfcat.get)).strip()] + int(words[1].strip())
            else:
...
if  __name__ =='__main__':CLASSIFY_LIBRARY_REDUCER()